...
const InputField: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label: string }> = ({ name, label, ...props }) => (
  <div>
    ...
    <input id={name} name={name} {...props} className="mt-1 block w-full px-3 py-2 border border-neutral-700 rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm bg-neutral-800 text-white placeholder-gray-400" />
  </div>
);
...
const TextAreaField: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }> = ({ name, label, ...props }) => (
  <div>
    <label htmlFor={name} className="block text-sm font-medium text-gray-300">{label}</label>
    <textarea id={name} name={name} rows={3} {...props} className="mt-1 block w-full px-3 py-2 border border-neutral-700 rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm bg-neutral-800 text-white placeholder-gray-400" />
  </div>
);
...
const SelectField: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label: string, options: {id: string | number, name: string}[] }> = ({ name, label, options, ...props }) => (
  <div>
    <label htmlFor={name} className="block text-sm font-medium text-gray-300">{label}</label>
    <select id={name} name={name} {...props} className="mt-1 block w-full px-3 py-2 border border-neutral-700 bg-neutral-800 text-white rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm">
...
export const BusinessForm: React.FC<BusinessFormProps> = ({ categories, onSubmit, initialData, onCancel }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (type === 'checkbox' && e.target instanceof HTMLInputElement) {
        ...
        // Fix: Handle numeric conversion for latitude and longitude.
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="p-4 border-l-4 border-red-500 bg-neutral-900">
       ...
       <div className="p-4 border-l-4 border-red-500 bg-neutral-900 mt-8">
        <h2 className="text-lg font-bold text-white">Datos del Negocio</h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      {/* Fix: Add form fields for address, latitude, and longitude. */}
      <TextAreaField name="address" label="Dirección" value={formData.address} onChange={handleChange} />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <InputField name="latitude" label="Latitud" type="number" step="any" value={formData.latitude} onChange={handleChange} />
        <InputField name="longitude" label="Longitud" type="number" step="any" value={formData.longitude} onChange={handleChange} />
      </div>
      ...
      {initialData && (
        <div>
          <label className="block text-sm font-medium text-gray-300">Vencimiento de la Promoción</label>
          <p className="mt-1 text-sm text-gray-200 bg-neutral-800 p-2 rounded-md">{new Date(initialData.promotionEndDate).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>
      )}

      <div className="flex items-center">
        <input type="checkbox" id="isFeatured" name="isFeatured" checked={formData.isFeatured} onChange={handleChange} className="h-4 w-4 text-red-600 border-neutral-700 rounded focus:ring-red-500 bg-neutral-800" />
        <label htmlFor="isFeatured" className="ml-2 block text-sm font-medium text-gray-300">¿Es un negocio destacado?</label>
      </div>
      <div className="flex justify-end space-x-4 pt-4 border-t border-neutral-800">
        <button type="button" onClick={onCancel} className="bg-neutral-700 text-white font-semibold py-2 px-4 rounded-lg hover:bg-neutral-600 transition-colors">Cancelar</button>
